...
class Loss:
    def accumulate_gradients(
            self, phase, real_img, real_c, real_class, gen_z, gen_c, gain, cur_nimg):  # to be overridden by subclass
...
class StyleGAN2Loss(Loss):
    ...
    def accumulate_gradients(
            self, phase, real_img, real_c, real_class, gen_z, gen_c, gain, cur_nimg):
        ...
        if phase in ['Gmain', 'Gboth']:
            with torch.autograd.profiler.record_function('Gmain_forward'):
                ...
                if 'shapenet_car' in self.G.synthesis.data_camera_mode or 'shapenet_chair' in self.G.synthesis.data_camera_mode \
                        or 'shapenet_motorbike' in self.G.synthesis.data_camera_mode or self.G.synthesis.data_camera_mode == 'renderpeople' or \
                        'shapenet_bed' in self.G.synthesis.data_camera_mode or 'shapenet_plane' in self.G.synthesis.data_camera_mode or \
                        'shapenet_plant' in self.G.synthesis.data_camera_mode or 'shapenet_vase' in self.G.synthesis.data_camera_mode or \
                ...
                gen_logits = self.run_D(gen_img, torch.cat([gen_c, camera_condition], 1), mask_pyramid=mask_pyramid)
        ...
        if phase in ['Dmain', 'Dboth']:
            with torch.autograd.profiler.record_function('Dgen_forward'):
                ...
                if 'shapenet_car' in self.G.synthesis.data_camera_mode or 'shapenet_chair' in self.G.synthesis.data_camera_mode \
                        ...
                        'shapenet_plant' in self.G.synthesis.data_camera_mode or 'shapenet_vase' in self.G.synthesis.data_camera_mode or \
                        self.G.synthesis.data_camera_mode == 'ts_house' or self.G.synthesis.data_camera_mode == 'ts_animal' or \
                        'all_shapenet' in self.G.synthesis.data_camera_mode:
                    camera_condition = torch.cat((gen_camera[-2], gen_camera[-1]), dim=-1)
                else:
                    camera_condition = None
                ...
                gen_logits = self.run_D(
                    gen_img, torch.cat([gen_c, camera_condition], 1), update_emas=True, mask_pyramid=mask_pyramid)
        ...
        if phase in ['Dmain', 'Dreg', 'Dboth']:
            ...
            with torch.autograd.profiler.record_function(name + '_forward'):
                ...
                real_logits = self.run_D(real_img_tmp, torch.cat([real_class, real_c], 1))
