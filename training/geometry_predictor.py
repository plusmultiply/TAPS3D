...
class MappingNetwork(torch.nn.Module):
    def __init__(
            ...
            f_dim=512,   # CLIP feature dimensionality
            clip_in_mn=False, # Use CLIP feature in the mapping network
            num_clip_layers=4,
    ):
        super().__init__()
        self.z_dim = z_dim
        ...
        self.f_dim = f_dim
        self.clip_in_mn = clip_in_mn
        self.num_clip_layers = num_clip_layers
        
        if self.clip_in_mn == True and self.f_dim > 0:
            _f_dim = (f_dim + w_dim) // 2
            # pre process text features
            self.clip_pre_0 = FullyConnectedLayer(f_dim, _f_dim, activation=activation, lr_multiplier=lr_multiplier, device=device)  
            self.clip_pre_1 = FullyConnectedLayer(_f_dim, _f_dim, activation=activation, lr_multiplier=lr_multiplier, device=device)
            self.clip_affine_0 = FullyConnectedLayer(_f_dim + w_dim, w_dim, bias_init=1)  # f([w, txt])
            clip_features_list = [z_dim + f_dim] + [z_dim] * (num_clip_layers)
            if self.num_clip_layers > 0:
                for idx in range(num_clip_layers):
                    in_features = clip_features_list[idx]
                    out_features = clip_features_list[idx + 1]
                    layer = FullyConnectedLayer(
                        in_features, out_features, activation=activation, lr_multiplier=lr_multiplier, device=device)
                    setattr(self, f'clip_fc{idx}', layer)
        ...
        if self.clip_in_mn == True and self.f_dim > 0 and self.num_clip_layers == 0:
            features_list = [z_dim + embed_features + f_dim] + [layer_features] * (num_layers - 1) + [w_dim]
        else:
            features_list = [z_dim + embed_features] + [layer_features] * (num_layers - 1) + [w_dim]
    ...
    def update_w_avg(self, device='device', c=None):
        ...
        # if self.c_dim != 0:
        #     trg = torch.randint(0, self.c_dim, (n_z, ), device=device)
        #     c = F.one_hot(trg, num_classes=self.c_dim)
        # if not c is None:
        #     c = c[:n_z]  #########################
        if c != None:
            fts = c
            index = random.choices(range(len(fts)), k=n_z)
            fts = fts[index].to(device)
        else:
            fts = torch.zeros([n_z, self.f_dim], device=device)
        ws = self.forward(z, None, fts)
        avg_ws = torch.mean(ws, dim=0)[0]
        self.w_avg = self.w_avg * 0.0 + avg_ws

    def forward(self, z, c, fts=None, truncation_psi=1, truncation_cutoff=None, update_emas=False):
        ...
        with torch.autograd.profiler.record_function('input'):
            ...
            if self.clip_in_mn == True and self.f_dim > 0 and fts is not None:
                fts = self.clip_pre_0(fts)
                fts = self.clip_pre_1(fts)
                fts = normalize_2nd_moment(fts.to(torch.float32))
                x = torch.cat([x, fts], dim=1) if x is not None else fts
                if self.num_clip_layers > 0:
                    for idx in range(self.num_clip_layers):
                        layer = getattr(self, f'clip_fc{idx}')
                        x = layer(x)
            if self.c_dim > 0:
