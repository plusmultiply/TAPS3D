...
def setup_snapshot_image_grid(training_set, random_seed=0, inference=False):
    ...
    if not training_set.has_labels:
        ...
        for y in range(gh):
            ...
            grid_classlabel += [F.one_hot(torch.tensor(y), num_classes=len(label_order))] * gw
        grid_classlabel = torch.stack(grid_classlabel)

    # Load data.
    images, labels, masks = zip(*[training_set[i][:3] for i in grid_indices])
    return (gw, gh), np.stack(images), np.stack(labels), masks, grid_classlabel
...
def training_loop(
    ...
    if rank == 0:
        ...
        grid_size, images, labels, masks, grid_classlables = setup_snapshot_image_grid(training_set=training_set, inference=inference_vis)
        ...
        if grid_classlables == None:
            ...
            grid_c = grid_classlables.to(device).split(1)
    ...
    while True:
        ...
        with torch.autograd.profiler.record_function('data_fetch'):
            phase_real_img, phase_real_c, real_mask, phase_real_class = next(training_set_iterator)
        ...
        for phase, phase_gen_z, phase_gen_c in zip(phases, all_gen_z, all_gen_c):
            ...
            for real_img, real_c, real_class, gen_z, gen_c in zip(phase_real_img, phase_real_c, phase_real_class, phase_gen_z, phase_gen_c):
                loss.accumulate_gradients(
                    phase=phase.name, real_img=real_img, real_c=real_c, real_class=real_class, gen_z=gen_z, gen_c=gen_c,
