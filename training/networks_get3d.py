...
class GeneratorDMTETMesh(torch.nn.Module):
    def __init__(
        ...
        if 'clip_in_texmn' in mapping_kwargs and 'clip_in_geomn' in mapping_kwargs:
            clip_in_texmn = mapping_kwargs['clip_in_texmn']
            clip_in_geomn = mapping_kwargs['clip_in_geomn']
            mapping_kwargs.pop('clip_in_texmn')
            mapping_kwargs.pop('clip_in_geomn')
            self.mapping = MappingNetwork(
                z_dim=z_dim, c_dim=c_dim, w_dim=w_dim, num_ws=self.num_ws,
                device=self.synthesis.device, clip_in_mn=clip_in_texmn, **mapping_kwargs)
            self.mapping_geo = MappingNetwork(
                z_dim=z_dim, c_dim=c_dim, w_dim=w_dim, num_ws=self.num_ws_geo,
                device=self.synthesis.device, clip_in_mn=clip_in_geomn, **mapping_kwargs)
        else:
            self.mapping = MappingNetwork(
                z_dim=z_dim, c_dim=c_dim, w_dim=w_dim, num_ws=self.num_ws,
    ...
    def generate_3d_mesh_text(
            self, geo_z, tex_z, c, clip_fts, truncation_psi=1, truncation_cutoff=None, update_emas=False,
            with_texture=True, use_style_mixing=False, use_mapping=True, **synthesis_kwargs):
        '''
        This function generates a 3D mesh with given geometry latent code (geo_z) and texture
        latent code (tex_z), it can also generate a texture map is setting `with_texture` to be True.
        ...
        if not with_texture:
            ...
            if use_mapping:
                ws_geo = self.mapping_geo(
                    geo_z, c, clip_fts, truncation_psi=truncation_psi, truncation_cutoff=truncation_cutoff,
                    update_emas=update_emas)
            else:
                ws_geo = geo_z
            if use_style_mixing:
            ...
            if self.synthesis.one_3d_generator:
                # For this model, we first generate the feature map for it
                ws_tex = self.mapping(geo_z, c, clip_fts, truncation_psi=truncation_psi)  # we didn't use it during inference
                sdf_feature, tex_feature = self.synthesis.generator.get_feature(
                    ws_tex[:, :self.synthesis.generator.tri_plane_synthesis.num_ws_tex],
                    ws_geo[:, :self.synthesis.generator.tri_plane_synthesis.num_ws_geo])
                ws_tex = ws_tex[:, self.synthesis.generator.tri_plane_synthesis.num_ws_tex:]
        ...
        if use_mapping:
            ws = self.mapping(
                tex_z, c, clip_fts, truncation_psi=truncation_psi, truncation_cutoff=truncation_cutoff, update_emas=update_emas)
            ws_geo = self.mapping_geo(
                geo_z, c, clip_fts, truncation_psi=truncation_psi, truncation_cutoff=truncation_cutoff,
                update_emas=update_emas)
        else:
        ...
        if use_style_mixing:
            ...
            cutoff = torch.where(
                ...
                torch.full_like(cutoff, ws.shape[1]))
            ws[:, cutoff:] = self.mapping(
                torch.randn_like(tex_z), c, clip_fts, update_emas=False, truncation_psi=truncation_psi,
                truncation_cutoff=truncation_cutoff
            )[:,
                             cutoff:]
        all_mesh = self.synthesis.extract_3d_shape(ws, ws_geo, )
