...
def save_textured_mesh_for_inference_text(
        G_ema, grid_z, grid_c, clip_fts, run_dir, save_mesh_dir=None,
    ...
    with torch.no_grad():
        ...
        for idx in range(len(grid_z)):
            geo_z = grid_z[idx].unsqueeze(0)
            clip_ft = clip_fts[idx].unsqueeze(0)
            if grid_tex_z is None:
                tex_z = grid_z[idx].unsqueeze(0)
            else:
                tex_z = grid_tex_z[idx].unsqueeze(0)
            generated_mesh = G_ema.generate_3d_mesh_text(
                geo_z=geo_z, tex_z=tex_z, c=None, clip_fts=clip_ft, truncation_psi=0.7,
