...
class CLIPLoss(torch.nn.Module):
    def __init__(self, device, lambda_direction=1., lambda_patch=0., lambda_global=0., lambda_manifold=0., lambda_texture=0., lambda_imgcos=0., patch_loss_type='mae', direction_loss_type='cosine', clip_model='ViT-B/32'):
        ...
        self.lambda_imgcos   = lambda_imgcos
    ...
    def global_clip_loss(self, img: torch.Tensor, text) -> torch.Tensor:
        ...
        return (1. - logits_per_image.diag() / 100).mean()
    ...
    def clip_imgcos_loss(self, src_img: torch.Tensor, target_img: torch.Tensor) -> torch.Tensor:
        # import pdb; pdb.set_trace()
        src_features = self.get_image_features(src_img)
        target_features = self.get_image_features(target_img)

        # cosine similarity as logits
        logits = src_features @ target_features.t()

        return (1. - logits.diag()).mean()
    ...
    # def forward(self, target_img: torch.Tensor, target_class: str, texture_image: torch.Tensor = None):
        clip_loss = 0.0
        ...
        if self.lambda_direction:
            if isinstance(source_class, list) == True:
                for (src_i, src_c, tar_i, tar_c) in zip(src_img, source_class, target_img, target_class):
                    clip_loss += self.lambda_direction * self.clip_directional_loss(src_i.unsqueeze(0), src_c, tar_i.unsqueeze(0), tar_c)
            else:
        ...
        if self.lambda_imgcos:
            clip_loss += self.lambda_imgcos * self.clip_imgcos_loss(src_img, target_img)
