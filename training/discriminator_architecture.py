...
class MappingNetwork(torch.nn.Module):
    ...
    def update_w_avg(self, device='device'):
        ...
        if self.c_dim != 0:
            trg = torch.randint(0, self.c_dim, (100000, ), device=device)
...
class Discriminator(torch.nn.Module):
    def __init__(
        ...
        if self.add_camera_cond:
            if 'shapenet_car' in self.data_camera_mode \
                    or 'shapenet_chair' in self.data_camera_mode or 'shapenet_motorbike' in self.data_camera_mode \
                    or 'shapenet_bed' in self.data_camera_mode or 'shapenet_plane' in self.data_camera_mode \
    ...
    def forward(self, img, c, clip_fts, update_emas=False, alpha=1.0, mask_pyramid=None, **block_kwargs):
